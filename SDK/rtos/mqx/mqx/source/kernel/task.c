...
)
{
    ...
    if (td_ptr != NULL)
    {
        /* To find out if the task should be created in blocked state we have to find its template structure. */
        if ( template_index )
        {
            ...
            assert (task_template_ptr->TASK_TEMPLATE_INDEX == template_index);
        }
        else
        {
            task_template_ptr = (TASK_TEMPLATE_STRUCT_PTR)parameter;
        }
        blocking = task_template_ptr->TASK_ATTRIBUTES & MQX_TASK_CREATE_BLOCKED;
        ...
        {
            /* If the task needs to run, check if we should reschedule */
            _INT_DISABLE();
...
#endif /* MQXCFG_ALLOCATOR != MQX_ALLOCATOR_NONE */
...
/*!
 * \brief Creates the task, but do not make it ready. Deprecated. The user better
 *        use the create_task with the MQX_TASK_CREATE_BLOCKED attribute of the task.
 *
 * \param[in] processor_number One of the following:
 * \n - Processor number of the processor where the task is to be created.
 * \n - 0 (Create on the local processor.)
...
)
{
    ...
    #if MQX_ALLOCATOR_GARBAGE_COLLECTING
    {
        ...
        while (block_ptr != NULL)
        {
            ...
            //need to uncomment it after include conficts with assert.h will be resolved
...
#if !MQX_CUSTOM_MAIN
// this check cannot be performed using MQX_CUSTOM_MAIN
// because init and iddle task were not enabled yet

#   if MQX_CHECK_ERRORS
#if MQXCFG_ALLOCATOR == MQX_ALLOCATOR_NONE
    #if MQXCFG_PREALLOCATED_SYSTEM_STACKS == 0
        #error There is not option to create stack. Preallocated stacks do not exist and dynamic allocation is dissabled.
    #endif
    assert (input_stack_ptr != NULL); /* Preallocated stacks cannot be used with NULL input_stack_ptr. */
#else /* MQXCFG_ALLOCATOR != 0 */
    assert ((input_stack_ptr != NULL) || (template_ptr->TASK_STACKSIZE != 0)); /* There must be defined stack location (preallocated stack) or input_stack_size (for dynamic stack allocation). */
#endif
...
{ /* Body */
    ...
    *overhead = extra;
    ...
    //assert(q_ptr->NEXT != NULL);
    //assert(q_ptr->PREV != NULL);
    ...
    do
    {
        ...
        //assert(insert_td_ptr->TD_LIST_INFO.NEXT != NULL);
...
/*!
 * \brief Creates the task and make it ready (depending on input parameters).
 *
 * \param[in] task_init_p Pointer to the task initialization structure (taskinit_t):
 * \n - .exec - Function address
 * \n - .stacksize - Size of the stack required to allocate (if allocation required)
 * \n - .stackaddr - Address of the stack (if allocation not required)
 * \n - .priority - Priority of the task
 * \n - .name - Human friendly name of the task
 * \n - .attributes - Attributes of the task, combination of MQX_AUTO_START_TASK,
 * \n   MQX_FLOATING_POINT_TASK, MQX_TIME_SLICE_TASK, MQX_DSP_TASK, MQX_USER_TASK,
 * \n   MQX_TASK_CREATE_BLOCKED.
 * \n - .exec_param - Execution parameter for the task
 * \n - .time_slice - Amount of time slices
 *
 * \return Task ID of the child task (Success.)
 * \return MQX_NULL_TASK_ID (Failure.)
 *
 ...
 * \warning If the child is on the same processor, preempts the creator if the
 * child is a higher priority.
 * \n - MQX_INVALID_TASK_PRIORITY (The task priority is not within expected range.)
 * \n - MQX_INVALID_SIZE (Invalid size of the stack.)
 *
 * \see _task_create_blocked
 * \see _task_create_at
 * \see _task_abort
...
(
    const taskinit_t * taskinit_p
)
{
    KERNEL_DATA_STRUCT_PTR  kernel_data;
    TD_STRUCT_PTR           td_ptr;
    _task_id                task_id = MQX_NULL_TASK_ID;
    int32_t                 status = 0;
    TASK_TEMPLATE_STRUCT    temp_template = {0};
    void *                  aligned_stack_address;
    _mem_size               reduced_stack_size;

#if MQXCFG_ALLOCATOR == MQX_ALLOCATOR_NONE
    /* Input stack must exist. */
    assert(taskinit_p->stackaddr != NULL);
#endif /* !MQXCFG_ALLOCATOR */

    // FIXME:
    // Lower functions are buggy, they do not correctly free
    // resources when error occurs.
    // So at this point we are trying to predict errors before
    // resources are allocated.
    _GET_KERNEL_DATA(kernel_data);

    // Invalid task priority
    status = ((kernel_data->LOWEST_TASK_PRIORITY) < (taskinit_p->priority));
    assert(!status);
    if (status)
    {
        return MQX_NULL_TASK_ID;
    ...
    // We already have maximum number of tasks in system
    status = (_QUEUE_GET_SIZE(&kernel_data->TD_LIST) >= (MAX_UINT_16-1));
    assert(!status);
    if (status)
    {
        return MQX_NULL_TASK_ID;
    ...
    temp_template.CREATION_PARAMETER    = (uint32_t)taskinit_p->exec_param;
    ...
    // Check whether created task is NULL
    status = (td_ptr == NULL);
    assert(!status);
    if (status)
    {
    ...
    task_id = td_ptr->TASK_ID;
    if ((kernel_data->ACTIVE_PTR) == (&kernel_data->SYSTEM_TD) ||
       (temp_template.TASK_ATTRIBUTES & MQX_TASK_CREATE_BLOCKED))
    {
        /* We will not make the task ready if the active task is
        ** system task (meaning scheduler has not started yet)
        ** or if the task was explicitly created in blocked state.
        */
        td_ptr->STATE = BLOCKED;
    }
    else
    {
        /* Scheduler is active, we can re-schedule to get higher priority task running */
        _INT_DISABLE();
        _task_ready_internal(td_ptr);
        _INT_ENABLE();
        _CHECK_RUN_SCHEDULER();
